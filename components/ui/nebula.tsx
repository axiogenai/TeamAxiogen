...
void main() {
    ...
    // Toned down the white wash-out to keep colors rich and deep
    color = mix(color, uColor1 * 1.4, clamp(length(r.x) * 0.4, 0.0, 1.0));
    color = mix(color, vec3(1.0), clamp(length(r.x) - 0.5, 0.0, 1.0));
...
interface NebulaProps {
  className?: string;
  speed?: number;
  color1?: string; // Highlights/Fracture (Orange/Sunset accent)
  color2?: string; // Nebula main (Cosmic purple/magenta)
  color3?: string; // Deep space (Deep dark space)
}
